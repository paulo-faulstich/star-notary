...
it('can add the star name and star symbol properly', async() => {
    ...
    let instance = await StarNotary.deployed();
    assert.equal(await instance.name.call(), "Acme Star");
    assert.equal(await instance.symbol.call(), "ACMES");
});
...
it('lets 2 users exchange stars', async() => {
    ...
    await instance.createStar('Scorpius', tokenId1, {from: user1})
    await instance.createStar('Andromeda', tokenId2, {from: user2})
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let user = accounts[0];
    let starName = "Scorpius";
    
    await instance.createStar(starName, tokenId, {from: user})
    
    let starNameLookedUp = await instance.lookUptokenIdToStarInfo(tokenId);
